...
import { DramaticCheck, DramaticZap, DramaticArrowRight, DramaticShield, DramaticTrophy, DramaticTarget, DramaticMonitor, DramaticShieldAlert, DramaticChart, DramaticClock } from '@/components/icons/dramatic-icons'
...
import { PricingBackground } from '@/components/3d/page-scenes'
      ...
      <section className="pt-48 pb-32">
        <Container size="2xl">
          ...
          <div className="flex flex-col items-center text-center space-y-8 mb-16">
            ...
            <WordReveal
              text="All test series are Remote Proctored — AI Based Proctoring, Secure & Cheat Proof, with Detailed Performance Analysis."
              className="text-xl md:text-2xl text-muted-foreground max-w-2xl font-clean font-medium opacity-80"
            />
          </div>

          <ScrollingText
            texts={['JEE MAINS', 'NEET-UG', 'VITEEE', 'BITSSAT', 'COMEDK', 'JEE ADVANCED', 'MHT-CET', 'KCET']}
            speed={40}
                  ...
                  <div className="flex-1 text-center md:text-left space-y-3">
                    ...
                    <MagneticButton>
                      <SparkButton>
                        ...
                        >
                          GET THIS OFFER — ₹99
                          <DramaticArrowRight className="w-5 h-5 ml-2" />
                        </LiquidButton>
                      </SparkButton>
                    </MagneticButton>
                  </div>
          ...
          {/* Plan Cards */}
          <StaggerGrid className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
            {plans.map((plan) => (
              <DramaticCard key={plan.id} glowColor="rgba(var(--primary-rgb),0.4)">
                <Card
                  variant="glass"
                        ...
                        {plan.features.map((feature) => (
                          <li key={feature} className="flex items-center gap-3">
                            <div className="w-5 h-5 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                              <DramaticCheck className="w-3 h-3 text-primary" />
                            </div>
                      ...
                      <MagneticButton>
                        <SparkButton>
                          <LiquidButton
                            ...
                            className={`w-full h-20 rounded-2xl font-black uppercase tracking-widest shadow-xl group-hover:scale-[1.02] transition-transform ${plan.popular ? 'bg-primary text-primary-foreground' : 'border-2 border-primary/30 gradient-border bg-transparent text-foreground'}`}
                          >
                            GET ACCESS
                            <DramaticArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
                          </LiquidButton>
                        </SparkButton>
                      </MagneticButton>
                    </CardContent>
          ...
          <ScrollingText
            texts={['CHAPTERWISE TESTS', 'FULL LENGTH TESTS', 'AI PROCTORING', 'RANK BOOSTER', 'DETAILED ANALYTICS', 'CHEAT PROOF']}
            speed={50}
            className="opacity-40 my-8"
            reverse
          />
          ...
          <div className="mt-20 mb-8">
            <div className="rounded-2xl border border-white/5 bg-white/[0.02] p-8 grid grid-cols-2 md:grid-cols-4 gap-8">
              {[
                { icon: DramaticMonitor, label: 'Chapterwise & Full Length Tests' },
                { icon: DramaticTarget, label: 'Exam Pattern Aligned' },
                { icon: DramaticChart, label: 'Detailed Solutions & Analytics' },
                { icon: DramaticTrophy, label: 'Rank Booster Series' },
              ].map((item, i) => (
          ...
          <StaggerGrid className="mt-12">
            <DramaticCard glowColor="rgba(var(--primary-rgb),0.4)">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 p-6 rounded-2xl">
                {[
                  { icon: DramaticShield, label: 'Secure Payment', val: 'AES-256' },
                  { icon: DramaticTrophy, label: 'Success Rate', val: '94.2%' },
                  { icon: DramaticShieldAlert, label: 'Cheat Proof', val: 'AI Proctor' },
                  { icon: DramaticZap, label: 'Uplink Speed', val: 'Sub-second' },
                ].map((b, i) => (
                  <motion.div
                    key={i}
                    initial={{ opacity: 0 }}
