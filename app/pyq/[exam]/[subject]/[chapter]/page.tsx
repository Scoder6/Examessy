...
import { BookCheck, ArrowLeft, Clock, CheckCircle2 } from 'lucide-react'
...
export default function ChapterPage() {
  ...
  const chapterName = decodeURIComponent(params.chapter as string)
  ...
  const chapterQuestions = questions.filter(
    (q) => q.chapter === chapterName && q.subject === subjectName
  )

  if (!exam || !chapterQuestions.length) {
    return (
      <main className="min-h-screen bg-background">
        ...
        <Container size="2xl" className="py-48">
          <Card className="p-12 text-center">
            <CardContent>
              <h2 className="text-2xl font-semibold mb-4">Chapter not found</h2>
              <Button onClick={() => router.push(`/pyq/${examId}/${subjectId}`)}>Back to Chapters</Button>
            </CardContent>
          </Card>
        </Container>
        <Footer />
      ...
      <section className="pt-48 pb-24">
        <Container size="2xl">
          ...
          >
            <Button
              variant="ghost"
              onClick={() => router.push(`/pyq/${examId}/${subjectId}`)}
              className="mb-4"
            >
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <Badge variant="outline" className="px-4 py-1.5 rounded-full text-xs font-semibold border-secondary/20 text-secondary">
                {chapterName}
              </Badge>
            </div>
            ...
            <p className="text-lg text-muted-foreground/70 max-w-2xl">
              {chapterQuestions.length} previous year questions from {exam.name} {subjectName} - {chapterName}.
            </p>
          </motion.div>
          ...
          <div className="space-y-6">
            {chapterQuestions.map((question, index) => (
              ...
              >
                <Card className="rounded-2xl border border-border-subtle bg-card hover:border-primary/30 transition-all duration-300 hover:shadow-lg">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-center gap-3 flex-wrap">
                        ...
                        <Badge variant="outline" className={`text-xs ${
                          question.difficulty === 'Easy' ? 'text-green-500 border-green-500/20' :
                          question.difficulty === 'Medium' ? 'text-yellow-500 border-yellow-500/20' :
                      ...
                      <Button
                        ...
                        className="text-xs"
                      >
                        {selectedQuestion === question.id ? 'Hide' : 'Show'} Solution
                      </Button>
                    </div>
                        ...
                        <div
                          ...
                          className={`p-4 rounded-xl border transition-all cursor-pointer ${
                            selectedQuestion === question.id
                              ...
                              : 'border-border-subtle bg-card hover:border-primary/30 hover:bg-card-bg-hover'
                          }`}
                          onClick={() => selectedQuestion !== question.id && setSelectedQuestion(question.id)}
                        >
                          <div className="flex items-center gap-3">
                            <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                              selectedQuestion === question.id && i === question.correctAnswer
                      ...
                      >
                        <div className="space-y-3">
                          <div className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            <span>Appeared in: {question.appearedIn.join(', ')}</span>
                          </div>
